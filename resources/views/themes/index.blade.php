            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="themeGrid">
                @forelse($themes as $theme)
                    <div class="theme-card rounded-2xl transition-all duration-500 ease-out hover:shadow-2xl hover:-translate-y-1.5 shadow-soft"
                        ...
                        <div class="rounded-2xl overflow-hidden border border-neutral-100 dark:border-secondary-700/50 group">
                        {{-- Full-card background image --}}
                        @if($theme->thumbnail_portrait)
                            <img src="{{ Storage::url($theme->thumbnail_portrait) }}" alt="{{ $theme->name }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105">
                        @endif
                        ...
                        <div class="relative z-10 flex flex-col">
                            <div class="relative aspect-[4/5]">
                                ...
                                <div class="absolute top-3 left-3 z-20">
                                    @if($theme->is_premium)
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-gradient-to-r from-amber-400 to-amber-500 text-white shadow-lg shadow-amber-200/50">
                                            <i class="fas fa-crown text-xs"></i>
                                            Premium
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-white/90 backdrop-blur-sm text-emerald-700 shadow-sm border border-emerald-200/50">
                            ...
                            <div class="p-5 bg-white/70 dark:bg-secondary-800/70 backdrop-blur-xl">
                                <h3 class="text-lg font-bold text-secondary-800 dark:text-neutral-200 group-hover:text-primary-600 transition-colors leading-snug mb-2">
                                    {{ $theme->name }}
                                </h3>
