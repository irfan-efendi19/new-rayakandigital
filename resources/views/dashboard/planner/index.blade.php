    ...
    <div class="min-h-screen">
        ...
        <div class="hero-mesh grain-overlay border-b border-neutral-200/60 dark:border-secondary-700/40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-8">
                ...
                @if($weddingDate)
                    ...
                    @endphp
                    <div
                        class="mt-6 rounded-2xl bg-gradient-to-r from-primary-600 via-primary-700 to-secondary-900 p-5 sm:p-6 text-white relative overflow-hidden">
                        <div class="absolute -right-8 -bottom-10 opacity-10 pointer-events-none">
                            <svg class="w-56 h-56" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        ...
                        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                            <div class="min-w-0">
                                <p class="text-xs uppercase tracking-widest text-white/70 font-semibold">Countdown Menuju
                                    Hari H</p>
                            ...
                            <div class="flex-shrink-0">
                                @if($isPast)
                                    <div
                                        class="flex items-center gap-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/15 px-4 py-3">
                                    ...
                                    <div class="grid grid-cols-4 gap-2 sm:gap-3"
                                        ...
                                        <template x-if="initialized">
                                            <template x-for="unit in [
                                                ...
                                                <div
                                                    class="bg-white/10 backdrop-blur-sm rounded-xl py-2.5 px-2 text-center border border-white/15 min-w-[64px]">
                                                    <p class="text-xl sm:text-2xl font-extrabold tabular-nums leading-none"
                                                        x-text="String(unit.value).padStart(2, '0')"></p>
                                                    <p class="text-[10px] uppercase tracking-wider text-white/70 mt-1"
                                                        x-text="unit.label"></p>
                                                </div>
                                            </template>
                                        </template>
                                        <template x-if="!initialized">
                                            <div
                                                class="col-span-4 flex items-center gap-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/15 px-4 py-3 text-sm font-semibold">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M5 13l4 4L19 7" />
                                                </svg>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <div
                    class="p-4 sm:p-5 bg-blue-50 border border-blue-200 rounded-2xl dark:bg-blue-950/30 dark:border-blue-800/50">
                    <p class="text-[11px] sm:text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">Total
                        Estimasi Anggaran</p>
                    <p class="text-xl sm:text-2xl font-bold text-blue-900 dark:text-blue-100 mt-1 tabular-nums">Rp
                        {{ number_format($totalEstimated, 0, ',', '.') }}
                    </p>
                    <p class="text-[11px] sm:text-xs text-blue-500 dark:text-blue-400 mt-1">
                        {{ $itemsByCategory['BUDGET']->count() + $itemsByCategory['VENDOR']->count() + $itemsByCategory['SESERAHAN']->count() + $itemsByCategory['ENGAGEMENT']->count() + $itemsByCategory['PRE_WEDDING']->count() }} item
                    </p>
                </div>
                <div
                    class="p-4 sm:p-5 bg-emerald-50 border border-emerald-200 rounded-2xl dark:bg-emerald-950/30 dark:border-emerald-800/50">
                    <p class="text-[11px] sm:text-xs text-emerald-600 dark:text-emerald-400 font-semibold uppercase tracking-wide">
                        Total Terbayar (DP/Lunas)</p>
                    <p class="text-xl sm:text-2xl font-bold text-emerald-900 dark:text-emerald-100 mt-1 tabular-nums">Rp
                        {{ number_format($totalPaid, 0, ',', '.') }}
                    </p>
                </div>
                <div
                    class="p-4 sm:p-5 bg-amber-50 border border-amber-200 rounded-2xl dark:bg-amber-950/30 dark:border-amber-800/50">
                    <p class="text-[11px] sm:text-xs text-amber-600 dark:text-amber-400 font-semibold uppercase tracking-wide">Sisa
                        Tagihan Vendor</p>
                    <p class="text-xl sm:text-2xl font-bold text-amber-900 dark:text-amber-100 mt-1 tabular-nums">Rp
            ...
            <div
                ...
                <div
                    class="px-4 sm:px-5 py-3 sm:py-4 border-b border-neutral-200/80 dark:border-secondary-700/60 flex items-center justify-between gap-3">
                    <div>
                        <h3 class="font-semibold text-secondary-800 dark:text-neutral-100 text-sm sm:text-base">Rundown Acara Hari H</h3>
                        <p class="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Time schedule kegiatan
                            ditampilkan secara kronologis.</p>
                    </div>
                ...
                @else
                    <div class="p-4 space-y-3">
                        @foreach($rundowns as $index => $rundown)
                            <div
                                class="group relative flex items-stretch rounded-xl border border-neutral-150 dark:border-secondary-600/50 bg-neutral-50/50 dark:bg-secondary-700/30 hover:border-primary/40 dark:hover:border-primary/40 hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-all duration-200">
                                <div
                                    class="flex items-center justify-center w-20 sm:w-24 shrink-0 border-r border-neutral-150 dark:border-secondary-600/50 bg-neutral-100/50 dark:bg-secondary-700/50 rounded-l-xl">
                                    <div class="text-center">
                                        ...
                                        @if($rundown->time_end)
                                            <p class="text-[10px] text-neutral-400 dark:text-neutral-500 tabular-nums mt-0.5">s.d.
                                ...
                                <div
                                    class="absolute left-20 sm:left-24 top-1/2 -translate-y-1/2 -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-primary border-2 border-white dark:border-secondary-800 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div x-data="{ activeTab: '{{ $pillars[0]['key'] }}' }"
                class="bg-white dark:bg-secondary-800 rounded-2xl border border-neutral-200/80 dark:border-secondary-700/60 overflow-hidden">
                {{-- Tabs --}}
                <div class="flex overflow-x-auto scrollbar-hide border-b border-neutral-200/80 dark:border-secondary-700/60">
                    @foreach($pillars as $pillar)
                        <button type="button" @click="activeTab = '{{ $pillar['key'] }}'"
                            class="flex-shrink-0 px-3 sm:px-4 py-3 text-xs sm:text-sm font-medium border-b-2 transition-colors whitespace-nowrap"
                            ...
                            {{ $pillar['label'] }}
                            <span
                                class="ml-1 sm:ml-1.5 inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold {{ $pillar['bg'] }} {{ $pillar['color'] }}">
                                ...
                                <div x-show="activeTab === '{{ $pillar['key'] }}'" x-cloak class="p-5">
                                    @if($pillar['key'] === 'CALENDAR')
                                        ...
                                        @if($weddingDate)
                                            <div
                                                ...
                                                <span class="inline-flex items-center gap-1.5"><span
                                                        class="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-sm shadow-amber-500/30"></span> Hari H</span>
                                                <span class="inline-flex items-center gap-1.5"><span
                                                        class="w-2.5 h-2.5 rounded-full bg-primary shadow-sm shadow-primary/30"></span> Hari ini</span>
                                                <span class="inline-flex items-center gap-1.5"><span
                                                        class="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/30"></span> Ada event</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($calendarItems->isNotEmpty())
                                            <div class="mt-5">
                                                <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-400 dark:text-neutral-500 mb-3">Jadwal Event</h4>
                                                <div class="space-y-2">
                                                    @foreach($calendarItems as $event)
                                                        <div class="group flex items-center gap-3 p-3 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-white dark:bg-secondary-800 hover:border-primary/30 dark:hover:border-primary/30 transition-all">
                                                            <div class="flex-shrink-0 w-10 h-10 rounded-lg bg-primary-50 dark:bg-primary-900/30 flex flex-col items-center justify-center">
                                                                <span class="text-[10px] font-bold text-primary-600 dark:text-primary-400 leading-none">{{ $event->event_date?->format('d') }}</span>
                                                                <span class="text-[8px] font-semibold text-primary-400 dark:text-primary-500 uppercase">{{ $event->event_date?->format('M') }}</span>
                                                            </div>
                                                            <div class="flex-1 min-w-0">
                                                                <p class="text-sm font-medium text-secondary-800 dark:text-neutral-100 truncate">{{ $event->title }}</p>
                                                                @if($event->description)
                                                                    <p class="text-[11px] text-neutral-400 dark:text-neutral-500 truncate">{{ $event->description }}</p>
                                                                @endif
                                                            </div>
                                    ...
                                    @elseif($pillar['key'] === 'CHECKLIST')
                                        ...
                                        <div
                                            ...
                                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                                <div class="min-w-0">
                                                    <p
                                                        class="text-xs uppercase tracking-widest text-primary dark:text-primary-400 font-semibold">
                                                        Checklist Wedding Plan</p>
                                                    <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-1">Item persiapan per
                                                        kategori.</p>
                                        ...
                                        @if($checklistTotalItems === 0)
                                            <div
                                                class="rounded-xl border border-dashed border-neutral-200 dark:border-secondary-600 px-5 py-10 text-center">
                                                <svg class="w-10 h-10 mx-auto text-neutral-300 dark:text-neutral-600" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                        d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                                                </svg>
                                                <p class="text-sm font-semibold text-secondary-800 dark:text-neutral-100 mt-3">Belum ada
                                                    checklist.</p>
                                                <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-1">Checklist persiapan
                                        ...
                                        @else
                                            ...
                                            <div class="space-y-5">
                                                @foreach($checklistCategories as $code => $label)
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="rounded-xl border border-neutral-200/80 dark:border-secondary-700/60 overflow-hidden">
                                                        <div
                                                            class="px-4 py-3 bg-neutral-50 dark:bg-secondary-700/40 border-b border-neutral-200/80 dark:border-secondary-700/60 flex items-center justify-between gap-3">
                                                            ...
                                                            @if($normalItems->isNotEmpty())
                                                                <ul class="divide-y divide-neutral-100 dark:divide-secondary-700/50">
                                                                    @foreach($normalItems as $item)
                                                                        <li
                                                                            class="px-4 py-2.5 flex items-center gap-3 hover:bg-neutral-50 dark:hover:bg-secondary-700/30 transition-colors">
                                                                            ...
                                                                            <div class="min-w-0 flex-1">
                                                                                ...
                                                                                @if($item->description)
                                                                                    <p
                                                                                        class="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5 line-clamp-2">
                                                                                        {{ $item->description }}
                                                                                    </p>
                                                                                @endif
                                                                            </div>
                                    ...
                                    @elseif($pillar['key'] === 'ADMINISTRATION')
                                        ...
                                        <div
                                            ...
                                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                                <div class="min-w-0">
                                                    <p
                                                        class="text-xs uppercase tracking-widest text-cyan-600 dark:text-cyan-400 font-semibold">
                                                        Administrasi & Legal</p>
                                                    <h3
                                                        class="font-heading text-lg font-bold text-secondary-800 dark:text-neutral-100 mt-1">
                                            ...
                                            <div class="mt-4">
                                                <div
                                                    class="h-2.5 bg-neutral-200/80 dark:bg-secondary-700/60 rounded-full overflow-hidden">
                                                    <div class="h-full bg-cyan-500 dark:bg-cyan-400 rounded-full transition-all duration-500"
                                                        :style="'width:' + categoryProgress('ADMINISTRATION') + '%'"></div>
                                                </div>
                                                <p class="text-right text-xs font-bold text-cyan-600 dark:text-cyan-400 mt-1 tabular-nums"
                                                                    ...
                                                                    <div class="flex-shrink-0 flex items-center gap-4">
                                                                        <label class="inline-flex items-center gap-1.5 cursor-pointer select-none">
                                                                            <input type="checkbox" :checked="items[{{ $item->id }}].pria"
                                                                                ...
                                                                                class="w-4 h-4 rounded border-neutral-300 dark:border-neutral-600 text-primary focus:ring-primary-500 cursor-pointer">
                                                                            <span
                                                                                class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Pria</span>
                                                                        </label>
                                                                        <label class="inline-flex items-center gap-1.5 cursor-pointer select-none">
                                                                            <input type="checkbox" :checked="items[{{ $item->id }}].wanita"
                                                                                ...
                                                                                class="w-4 h-4 rounded border-neutral-300 dark:border-neutral-600 text-primary focus:ring-primary-500 cursor-pointer">
                                                                            <span
                                                                                class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Wanita</span>
                                                                         </label>
                                                                     </div>
                                                                 </li>
                                                             @endforeach
                                    ...
                                    @elseif($pillar['key'] === 'VENDOR')
                                        ...
                                        <div class="flex items-center justify-between gap-3 mb-4">
                                            <div>
                                                ...
                                                <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{{ $totalVendors }}
                                                    vendor terdaftar</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                                            <div class="p-3 rounded-xl bg-neutral-100 dark:bg-secondary-700/50 text-center">
                                                <p class="text-[10px] text-neutral-500 dark:text-neutral-400">Total</p>
                                                <p class="text-sm font-bold text-secondary-800 dark:text-neutral-100 tabular-nums">
                                                    {{ $totalVendors }}</p>
                                            </div>
                                            <div class="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 text-center">
                                            ...
                                            <div class="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/30 text-center">
                                            ...
                                            <div class="p-3 rounded-xl bg-orange-50 dark:bg-orange-900/30 text-center">
                                                <p class="text-[10px] text-orange-500 dark:text-orange-400">Estimasi</p>
                                                <p class="text-sm font-bold text-orange-600 dark:text-orange-400 tabular-nums">Rp
                                                    {{ number_format($totalEstimate, 0, ',', '.') }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            @foreach(\App\Models\WeddingPlannerItem::VENDOR_TYPES as $type => $vendorLabel)
                                                ...
                                                @endphp
                                                <div
                                                    class="rounded-xl border border-neutral-200/80 dark:border-secondary-700/60 bg-white dark:bg-secondary-800 overflow-hidden">
                                                    {{-- Type Header --}}
                                                    <div class="px-4 py-3 border-b border-neutral-100 dark:border-secondary-700/50">
                                                        <div class="flex items-center justify-between">
                                                    ...
                                                    @if($vendors->isEmpty())
                                                        <div class="px-4 py-6 text-center">
                                                            <svg class="w-8 h-8 mx-auto text-neutral-300 dark:text-secondary-600 mb-2"
                                                    ...
                                                    @else
                                                        <div class="p-2 space-y-1.5">
                                                            @foreach($vendors as $vendor)
                                                                <div
                                                                    class="group relative flex items-center gap-3 p-2.5 rounded-lg hover:bg-neutral-50 dark:hover:bg-secondary-700/30 transition-colors">
                                                                    <div class="flex-1 min-w-0">
                                                                        <div class="flex items-center gap-2">
                                                                            <p
                                                                                ...
                                                                                {{ $vendor->title }}</p>
                                                                            <span
                                                                                class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold {{ $statusStyles[$vendor->status] ?? $statusStyles['PENDING'] }} shrink-0">
                                                                                {{ $statusLabels[$vendor->status] ?? $vendor->status }}
                                                                            </span>
                                                                        </div>
                                                                        @if($vendor->vendor_contact)
                                                                            <p
                                                                                class="text-[11px] text-neutral-400 dark:text-neutral-500 mt-0.5 flex items-center gap-1 truncate">
                                                                                <svg class="w-3 h-3 shrink-0" fill="none" viewBox="0 0 24 24"
                                                                                    stroke="currentColor">
                                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                                                        stroke-width="2"
                                    ...
                                    @elseif($pillar['key'] === 'ENGAGEMENT')
                                        ...
                                        <div class="grid grid-cols-3 gap-2 mb-4">
                                            <div class="p-3 rounded-xl bg-neutral-100 dark:bg-secondary-700/50 text-center">
                                                <p class="text-[10px] text-neutral-500 dark:text-neutral-400">Total</p>
                                                <p class="text-sm font-bold text-secondary-800 dark:text-neutral-100 tabular-nums">Rp
                                                    {{ number_format($engTotal, 0, ',', '.') }}</p>
                                            </div>
                                            <div class="p-3 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-center">
                                                <p class="text-[10px] text-blue-500 dark:text-blue-400">Pria</p>
                                                <p class="text-sm font-bold text-blue-600 dark:text-blue-400 tabular-nums">Rp
                                                    {{ number_format($engTotalPria, 0, ',', '.') }}</p>
                                            </div>
                                            <div class="p-3 rounded-xl bg-pink-50 dark:bg-pink-900/30 text-center">
                                                <p class="text-[10px] text-pink-500 dark:text-pink-400">Wanita</p>
                                                <p class="text-sm font-bold text-pink-600 dark:text-pink-400 tabular-nums">Rp
                                                    {{ number_format($engTotalWanita, 0, ',', '.') }}</p>
                                            </div>
                                        </div>

                                        @if($engItems->isEmpty())
                                            <div
                                                class="py-8 text-center text-sm text-neutral-400 dark:text-neutral-500 border border-dashed border-neutral-200 dark:border-secondary-600 rounded-xl">
                                        ...
                                        @else
                                            <div class="space-y-2">
                                                @foreach($engItems as $item)
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="group relative rounded-xl border border-neutral-200/70 dark:border-secondary-600/50 bg-white dark:bg-secondary-800 hover:border-pink-300 dark:hover:border-pink-700/50 transition-all duration-200">
                                                        <div class="px-4 py-3">
                                    ...
                                    @elseif($pillar['key'] === 'PRE_WEDDING')
                                        ...
                                        <div class="flex items-center justify-between gap-3 mb-4">
                                            <div>
                                                <h3 class="font-semibold text-secondary-800 dark:text-neutral-100 text-sm sm:text-base">Pre-Wedding</h3>
                                        ...
                                        @else
                                            ...
                                            <div class="grid grid-cols-3 gap-2 sm:gap-3 mb-5">
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-neutral-50/60 dark:bg-secondary-700/30">
                                                    <span class="text-[10px] sm:text-[11px] text-neutral-400 dark:text-neutral-500">Budget</span>
                                                    <p
                                                        class="text-sm sm:text-lg font-bold text-secondary-800 dark:text-neutral-100 tabular-nums mt-0.5">
                                                        Rp {{ number_format($preTotalBudget, 0, ',', '.') }}</p>
                                                </div>
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-emerald-200/70 dark:border-emerald-800/50 bg-emerald-50/60 dark:bg-emerald-900/20">
                                                    <span class="text-[10px] sm:text-[11px] text-emerald-500 dark:text-emerald-400">Bayar</span>
                                                ...
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-amber-200/70 dark:border-amber-800/50 bg-amber-50/60 dark:bg-amber-900/20">
                                                    <span class="text-[10px] sm:text-[11px] text-amber-500 dark:text-amber-400">Sisa</span>
                                                    <p class="text-lg font-bold text-amber-600 dark:text-amber-400 tabular-nums mt-0.5">Rp
                                            ...
                                            <div class="space-y-2.5">
                                                @foreach($preItems as $item)
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="group relative flex items-stretch rounded-xl border border-neutral-150 dark:border-secondary-600/50 bg-neutral-50/50 dark:bg-secondary-700/30 hover:border-violet-300 dark:hover:border-violet-700/50 hover:bg-violet-50/30 dark:hover:bg-violet-900/10 transition-all duration-200">
                                                        <div
                                    ...
                                    @elseif($pillar['key'] === 'SESERAHAN')
                                        ...
                                        <div x-data="{ sesFilter: 'ALL' }">
                                            <div class="flex items-center justify-between gap-3 mb-4">
                                                <div>
                                                    <h3 class="font-semibold text-secondary-800 dark:text-neutral-100 text-sm sm:text-base">Seserahan</h3>
                                                    <p class="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                            ...
                                            {{-- Toggle Filter --}}
                                            <div
                                                class="flex items-center gap-1 p-1 bg-neutral-100 dark:bg-secondary-700/50 rounded-xl mb-5">
                                            ...
                                            <div class="grid grid-cols-3 gap-2 sm:gap-3 mb-5">
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-neutral-50/60 dark:bg-secondary-700/30">
                                                    <span class="text-[10px] sm:text-[11px] text-neutral-400 dark:text-neutral-500">Total</span>
                                                    <p
                                                        class="text-sm sm:text-lg font-bold text-secondary-800 dark:text-neutral-100 tabular-nums mt-0.5">
                                                        Rp {{ number_format($sesTotal, 0, ',', '.') }}</p>
                                                </div>
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-blue-200/70 dark:border-blue-800/50 bg-blue-50/60 dark:bg-blue-900/20">
                                                    <span class="text-[10px] sm:text-[11px] text-blue-500 dark:text-blue-400">Pria</span>
                                                    <p class="text-sm sm:text-lg font-bold text-blue-600 dark:text-blue-400 tabular-nums mt-0.5">Rp
                                                        {{ number_format($sesTotalPria, 0, ',', '.') }}
                                                    </p>
                                                </div>
                                                <div
                                                    class="p-2.5 sm:p-3.5 rounded-xl border border-pink-200/70 dark:border-pink-800/50 bg-pink-50/60 dark:bg-pink-900/20">
                                                    <span class="text-[10px] sm:text-[11px] text-pink-500 dark:text-pink-400">Wanita</span>
                                                    <p class="text-sm sm:text-lg font-bold text-pink-600 dark:text-pink-400 tabular-nums mt-0.5">Rp
                                                ...
                                                <div class="space-y-4">
                                                    @foreach(\App\Models\WeddingPlannerItem::SESERAHAN_PARTIES as $partyCode => $partyLabel)
                                                        <div x-show="sesFilter === 'ALL' || sesFilter === '{{ $partyCode }}'"
                                                            ...
                                                            @endphp
                                                            <div
                                                                class="rounded-2xl border {{ $partyStyle['border'] }} overflow-hidden bg-white dark:bg-secondary-800">
                                                                <div
                                                                ...
                                                                @else
                                                                    <div class="p-3 space-y-2">
                                                                        @foreach($partyItems as $item)
                                                                            <div
                                                                                class="group relative flex items-stretch rounded-xl border border-neutral-150 dark:border-secondary-600/50 bg-neutral-50/50 dark:bg-secondary-700/30 hover:border-{{ $partyCode === 'PRIA' ? 'blue' : 'pink' }}-300 dark:hover:border-{{ $partyCode === 'PRIA' ? 'blue' : 'pink' }}-700/50 hover:bg-{{ $partyCode === 'PRIA' ? 'blue' : 'pink' }}-50/30 dark:hover:bg-{{ $partyCode === 'PRIA' ? 'blue' : 'pink' }}-900/10 transition-all duration-200">
                                                                                <div
                                    ...
                                    @elseif($pillar['key'] === 'BUDGET')
                                                ...
                                                <div x-data="{ budgetFilter: 'ALL' }">
                                                    <div class="flex items-center justify-between gap-3 mb-4">
                                                        <div>
                                                            <h3 class="font-semibold text-secondary-800 dark:text-neutral-100 text-sm sm:text-base">Anggaran
                                                                Pernikahan</h3>
                                                            <p class="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Kelola anggaran per
                                                                kategori.</p>
                                                        </div>
                                                        <button type="button" x-data
                                                    ...
                                                    {{-- Summary Cards --}}
                                                    <div class="grid grid-cols-3 gap-2 sm:gap-3 mb-5">
                                                        <div class="p-2.5 sm:p-3.5 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-neutral-50/60 dark:bg-secondary-700/30">
                                                            <span class="text-[10px] sm:text-[11px] text-neutral-400 dark:text-neutral-500">Estimasi</span>
                                                            <p class="text-sm sm:text-lg font-bold text-secondary-800 dark:text-neutral-100 tabular-nums mt-0.5">
                                                                Rp {{ number_format($grandTotalEstimated, 0, ',', '.') }}</p>
                                                        </div>
                                                        <div class="p-2.5 sm:p-3.5 rounded-xl border border-emerald-200/70 dark:border-emerald-800/50 bg-emerald-50/60 dark:bg-emerald-900/20">
                                                        ...
                                                        <div class="p-2.5 sm:p-3.5 rounded-xl border border-amber-200/70 dark:border-amber-800/50 bg-amber-50/60 dark:bg-amber-900/20">
                                                            <span class="text-[10px] sm:text-[11px] text-amber-500 dark:text-amber-400">Sisa Vendor</span>
                                                            <p class="text-sm sm:text-lg font-bold text-amber-600 dark:text-amber-400 tabular-nums mt-0.5">
                                                                Rp {{ number_format($vendorTotalRemaining, 0, ',', '.') }}</p>
                                                        </div>
                                                    </div>

                                                    {{-- Progress Bar --}}
                                                    <div class="mb-5 p-3 sm:p-4 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-neutral-50/60 dark:bg-secondary-700/30">
                                                        <div class="flex items-center justify-between mb-2">
                                                            <span class="text-[11px] sm:text-xs font-semibold text-secondary-800 dark:text-neutral-100">Progres Pembayaran</span>
                                                            <span class="text-[11px] sm:text-xs font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">{{ $grandPaidPercent }}%</span>
                                                        </div>
                                                        <div
                                                    ...
                                                    {{-- Toggle Filter --}}
                                                    <div
                                                        class="flex items-center gap-1 p-1 bg-neutral-100 dark:bg-secondary-700/50 rounded-xl mb-5 overflow-x-auto">
                                                    ...
                                                    @else
                                                                                                                    ...
                                                                                                                    <div class="flex items-center gap-2 flex-wrap">
                                                                                                                     ...
                                                                                                                     <div class="flex items-center gap-3 mt-2 flex-wrap">
                                                                                                                         <div class="flex items-center gap-1.5">
                                                                                                                             ...
                                                                                                                             <span class="text-xs font-semibold text-secondary-700 dark:text-neutral-200 tabular-nums">Rp {{ number_format($item->estimated_cost, 0, ',', '.') }}</span>
                                                                                                                         </div>
                                                                                                                         <div class="flex items-center gap-1.5">
                                                                                                                             <span class="text-[10px] text-emerald-400 dark:text-emerald-500">Bayar:</span>
                                                                                                                             <span class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 tabular-nums">Rp {{ number_format($item->paid_amount, 0, ',', '.') }}</span>
                                                                                                                         </div>
                                                                                                                     </div>
                                                                                                                     <div class="mt-2 flex items-center gap-2">
                                                                                                                         <div class="flex-1 h-1.5 bg-neutral-200 dark:bg-secondary-600 rounded-full overflow-hidden">
                                                                                                                             <div class="h-full bg-emerald-500 rounded-full" style="width: {{ $itemPaidPercent }}%"></div>
                                                                                                                         </div>
                                                                                                                         <span class="text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 tabular-nums">{{ $itemPaidPercent }}%</span>
                                                                                                                     </div>
                                                                                                                 </div>
                                                                                                        <div
                                                                                                            class="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                                                        ...
                                                        {{-- Total Summary --}}
                                                        <div class="mt-5 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200/60 dark:border-emerald-700/40">
                                                            <div class="flex items-center justify-between">
                                                                <div>
                                                                    <p class="text-sm font-bold text-secondary-800 dark:text-neutral-100">Total Anggaran</p>
                                                                    <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Budget Rp {{ number_format($grandTotalEstimated, 0, ',', '.') }} · Bayar Rp {{ number_format($grandTotalPaid, 0, ',', '.') }}</p>
                                                                </div>
                                                                <p class="text-xl font-bold text-emerald-700 dark:text-emerald-300 tabular-nums">Rp {{ number_format($grandTotalRemaining, 0, ',', '.') }}</p>
                                                            </div>
                                                        </div>
                                                    @endif
                                        </div>
                                ...
                                @else
                                    <div class="space-y-2.5">
                                        @foreach($itemsByCategory[$pillar['key']] as $item)
                                            <div
                                                class="flex items-start gap-3 p-3.5 rounded-xl border border-neutral-200/70 dark:border-secondary-700/50 bg-neutral-50/60 dark:bg-secondary-700/30">
                                                <div class="min-w-0 flex-1">
                                                    <div class="flex items-center gap-2 flex-wrap">
                ...
                <form action="{{ route('dashboard.planner.items.update', $item) }}" method="POST" class="space-y-4">
                    ...
                    @if($item->category === 'CALENDAR')
                        <div>
                            ...
                            <x-text-input id="edit-event-date-{{ $item->id }}" name="event_date" type="date" class="mt-1 block w-full"
        ...
        <div x-show="show" @click.away="show = false" @keydown.escape.window="show = false"
            ...
            <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                ...
                <div class="p-6">
                    ...
                    <p class="text-xs text-neutral-500 dark:text-neutral-400 mb-5">Tambahkan jadwal atau catatan ke kalender.</p>
