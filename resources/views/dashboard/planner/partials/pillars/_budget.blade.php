@php
    ...
    $budgetPaidPercent = $budgetTotalEstimated > 0 ? min(100, max(0, round(($budgetTotalPaid / $budgetTotalEstimated) * 100))) : 0;
    ...
    $grandPaidPercent = $grandTotalEstimated > 0 ? min(100, max(0, round(($grandTotalPaid / $grandTotalEstimated) * 100))) : 0;
...
<div x-data="{ budgetFilter: 'ALL' }">
    <div class="relative mb-5 overflow-hidden rounded-[28px] border border-emerald-200/70 bg-gradient-to-br from-emerald-700 via-teal-700 to-cyan-800 p-5 text-white shadow-[0_20px_50px_-24px_rgba(4,120,87,0.65)] dark:border-emerald-800/50 sm:p-6">
        ...
        <div class="flex items-center gap-4">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/15 ring-1 ring-white/20 backdrop-blur-sm">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 7h18v12H3V7zm0 4h18M7 15h3" /></svg>
            </div>
            <div>
            <p class="text-[10px] font-bold uppercase tracking-[0.24em] text-emerald-100">Kontrol Finansial</p>
            <h3 class="mt-1 font-heading text-xl font-bold sm:text-2xl">Anggaran Pernikahan</h3>
            <p class="mt-1 text-xs text-emerald-100/85">{{ $budgetItems->count() }} item dalam {{ $budgetGroups->count() }} kategori aktif.</p>
            </div>
        </div>
        <button type="button" x-data @click="setActiveTab('BUDGET'); $dispatch('open-modal', 'add-item-BUDGET')"
            class="inline-flex items-center gap-1.5 self-start rounded-xl bg-white px-3.5 py-2 text-xs font-bold text-emerald-800 shadow-sm transition-all hover:-translate-y-0.5 hover:bg-emerald-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-emerald-700 sm:self-auto">
    ...
    <div class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        <div
            ...
            <span class="text-[10px] text-neutral-400 dark:text-neutral-500 sm:text-[11px]">Estimasi kategori budget</span>
        ...
        <div
            ...
            <span class="text-[10px] sm:text-[11px] text-emerald-500 dark:text-emerald-400">Sudah terbayar</span>
        ...
        <div
            ...
            <p class="mt-0.5 text-sm font-bold text-amber-600 dark:text-amber-400 tabular-nums sm:text-lg">
                Rp {{ number_format($budgetTotalRemaining, 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div
        ...
        <div class="flex items-center justify-between mb-2">
            <span class="text-[11px] sm:text-xs font-semibold text-secondary-800 dark:text-neutral-100">Pembayaran kategori budget</span>
            <span
    ...
    <div class="mb-5 flex items-center gap-1 overflow-x-auto rounded-2xl bg-neutral-100 p-1 dark:bg-secondary-700/50">
        <button type="button" @click="budgetFilter = 'ALL'"
            :aria-pressed="(budgetFilter === 'ALL').toString()"
        ...
        @foreach($budgetGroups as $groupCode => $group)
            <button type="button" @click="budgetFilter = '{{ $groupCode }}'"
                :aria-pressed="(budgetFilter === '{{ $groupCode }}').toString()"
    ...
    @if($budgetItems->isEmpty())
        <div class="rounded-3xl border border-dashed border-emerald-200 bg-emerald-50/40 px-5 py-10 text-center dark:border-emerald-800/50 dark:bg-emerald-950/15">
            <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-emerald-600 shadow-sm dark:bg-secondary-800 dark:text-emerald-300">
                <i class="fa-solid fa-wallet" aria-hidden="true"></i>
            </span>
            <p class="mt-3 text-sm font-semibold text-secondary-800 dark:text-neutral-100">Belum ada anggaran</p>
            <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">Catat estimasi dan pembayaran agar sisa biaya terlihat otomatis.</p>
            <button type="button" x-data @click="$dispatch('open-modal', 'add-item-BUDGET')"
                class="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3.5 py-2 text-xs font-bold text-white transition hover:bg-emerald-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-secondary-800">
                <i class="fa-solid fa-plus text-[10px]" aria-hidden="true"></i> Tambah anggaran pertama
            </button>
        </div>
    @else
        <div class="flex flex-col gap-4">
            @foreach($budgetGroups as $groupCode => $group)
                @php
                    ...
                    $groupPaidPercent = $groupEstimated > 0 ? min(100, max(0, round(($groupPaid / $groupEstimated) * 100))) : 0;
                @endphp
                <div x-show="budgetFilter === 'ALL' || budgetFilter === '{{ $groupCode }}'"
                    ...
                    <div class="flex flex-col gap-2 p-3">
                        @foreach($group['items'] as $item)
                            ...
                            @endphp
                            <div
                                class="group relative rounded-xl border border-neutral-200 dark:border-secondary-600/50 bg-neutral-50/50 dark:bg-secondary-700/30 hover:border-emerald-300 dark:hover:border-emerald-700/50 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-all duration-200">
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        ...
                                        <div class="flex shrink-0 items-center gap-1">
                                            ...
                                            <form action="{{ route('dashboard.planner.items.destroy', $item) }}" method="POST"
                                                onsubmit="return confirmSwal(event, 'Hapus item budget ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
        ...
        <div
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm font-bold text-secondary-800 dark:text-neutral-100">Total seluruh modul keuangan</p>
                    <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Termasuk budget, vendor, lamaran, seserahan, dan pre-wedding · Estimasi Rp
                        {{ number_format($grandTotalEstimated, 0, ',', '.') }} · Bayar Rp
                        {{ number_format($grandTotalPaid, 0, ',', '.') }}
                    </p>
                </div>
                <div class="shrink-0 sm:text-right">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-emerald-600/70 dark:text-emerald-300/70">Belum terbayar</p>
                    <p class="mt-0.5 text-xl font-bold text-emerald-700 dark:text-emerald-300 tabular-nums">Rp {{ number_format($grandTotalRemaining, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    @endif
