    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($activeMethod === 'manual_bank')
                <div class="mb-6 bg-blue-50 border border-blue-200 text-blue-700 px-5 py-4 rounded-2xl text-sm flex items-center gap-3">
            ...
            @else
                <div class="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-700 px-5 py-4 rounded-2xl text-sm flex items-center gap-3">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {{-- Current Tier --}}
            <div class="mb-8 bg-white rounded-2xl shadow-soft border border-neutral-100 p-5 flex items-center justify-between">
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                @forelse($packages as $pkg)
                    ...
                    ">
                        @if($pkg->is_popular)
                            <div class="absolute top-0 left-0 right-0 bg-gradient-to-r from-primary to-primary-600 text-center py-2.5 z-10">
                        ...
                        <div class="p-8 {{ $pkg->is_popular ? 'pt-14' : '' }} flex-1">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="font-heading text-xl font-bold {{ $pkg->is_popular ? 'text-primary' : 'text-secondary-800' }}">
                                    {{ $pkg->package_name }}
                                </h3>
                                @if($currentTier === $pkg->package_code)
                                    <span class="text-xs font-bold px-3 py-1 rounded-full text-primary-700 bg-primary-50">AKTIF</span>
                                @endif
                            </div>
                            @if($pkg->description)
                            ...
                            <div class="mb-8">
                                @if($pkg->slashed_price && $pkg->slashed_price > $pkg->price)
                                    <span class="text-lg text-neutral-300 line-through">Rp {{ number_format($pkg->slashed_price, 0, ',', '.') }}</span>
                                @endif
                                <div class="flex items-baseline gap-1">
                                    <span class="text-4xl font-extrabold text-secondary-800">Rp {{ number_format($pkg->price, 0, ',', '.') }}</span>
                                    <span class="text-sm text-neutral-500">/ {{ $pkg->active_period_days === 0 ? 'Lifetime' : $pkg->active_period_days . ' Hari' }}</span>
                                </div>
                            </div>
