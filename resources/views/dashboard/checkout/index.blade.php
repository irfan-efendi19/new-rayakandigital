                        ...
                        <div class="p-6 {{ $pkg->is_popular ? 'pt-10' : '' }} flex flex-col flex-1 overflow-hidden">
                            ...
                            <div class="mt-6">
                                <div class="flex items-baseline gap-1">
                                    <span class="text-2xl font-bold text-secondary-900 dark:text-neutral-100">Rp</span>
                                    <span class="text-4xl font-extrabold text-secondary-900 dark:text-neutral-100 tracking-tight">
                                        {{ number_format($pkg->price, 0, ',', '.') }}
                                    </span>
                                    @if($pkg->price > 0)
                                        <span class="text-xs text-neutral-400 dark:text-neutral-500 font-medium ml-0.5">
                                ...
                                @if($pkg->slashed_price && $pkg->slashed_price > $pkg->price)
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-xs text-neutral-400 line-through">Rp {{ number_format($pkg->slashed_price, 0, ',', '.') }}</span>
                                        <span class="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-md">
                                            Hemat {{ number_format($pkg->slashed_price - $pkg->price, 0, ',', '.') }}
                                        </span>
                                    </div>
                                @endif
                            </div>
            ...
            async handleSubmit(e) {
                ...
                try {
                    ...
                    snap.pay(data.snap_token, {
                        onSuccess: () => window.location.href = '{{ $invitation ? route("dashboard.invitations.show", $invitation) : route("dashboard") }}',
                        onPending: () => window.location.href = '{{ route("dashboard") }}',
                        onError: () => { Swal.fire({ icon: 'error', title: 'Pembayaran Gagal', text: 'Silakan coba lagi.' }); },
                    });
                } catch (err) {
