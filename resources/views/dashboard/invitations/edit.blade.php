        ...
        return {
            ...
            saveCategory() {
                ...
                const url = this.editing ?
                    '{{ route("dashboard.invitations.guest-categories.update", [$invitation, '
                CATEGORY_ID ']) }}'.replace('CATEGORY_ID', this.form.id):
            ...
            deleteCategory(category) {
                Swal.fire({
                    ...
                    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content;
                    fetch('{{ route("dashboard.invitations.guest-categories.destroy", [$invitation, '
                            CATEGORY_ID ']) }}'.replace('CATEGORY_ID', category.id), {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
