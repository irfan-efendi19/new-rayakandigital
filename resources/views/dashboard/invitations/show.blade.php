<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            @if($invitation->hasFeature('personal_link'))
                                <a href="{{ route('dashboard.invitations.guests.index', $invitation) }}"
                                    class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 text-xs font-semibold">Kelola &rarr;</a>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-300">Premium</span>
                            @endif
                        </div>
                        <div class="text-3xl font-bold text-secondary-800 dark:text-neutral-100">{{ $invitation->guests->count() }}</div>
                ...
                <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border border-neutral-100 dark:border-secondary-700 p-5">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-xs text-neutral-400 dark:text-neutral-500">Total: {{ $invitation->rsvps->sum('pax') }} pax</span>
                    </div>
                ...
                <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border border-neutral-100 dark:border-secondary-700 p-5">
                    ...
                    <div class="text-3xl font-bold text-secondary-800 dark:text-neutral-100">{{ $invitation->wishes->count() }}</div>
                            ...
                            <div>
                                <h3 class="text-sm font-semibold text-secondary-800 dark:text-neutral-100 flex items-center gap-2">
                                    ...
                                    @if(!$invitation->hasFeature('qr_checkin'))
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-300">Platinum</span>
                                    @endif
                                </h3>
            ...
            <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border border-neutral-100 dark:border-secondary-700 overflow-hidden">
                <div class="p-5">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-sm font-semibold text-secondary-800 dark:text-neutral-100">Statistik Pengunjung (30 Hari Terakhir)</h3>
            ...
            <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border overflow-hidden
                {{ $videoLocked ? 'border-amber-100 dark:border-amber-800 bg-amber-50/20 dark:bg-amber-900/10' : 'border-neutral-100 dark:border-secondary-700' }}">
                <div class="p-5">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-sm font-semibold text-secondary-800 dark:text-neutral-100 flex items-center gap-2">
                            <svg class="w-4 h-4 text-primary dark:text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                            Video YouTube & Live Streaming
                            @if($videoLocked)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-300">Gold</span>
                            @endif
                        </h3>
                        @if(!$videoLocked && $invitation->youtube_video_id)
                            <span class="text-xs text-green-600 dark:text-green-400 font-semibold flex items-center gap-1">
                                <span class="w-1.5 h-1.5 rounded-full bg-green-500 dark:bg-green-400 inline-block"></span>
                                Terhubung
                            </span>
                        @endif
                    </div>
                    ...
                    @if(!$videoLocked)
                        <div class="space-y-4">
                            <form action="{{ route('dashboard.invitations.update', $invitation) }}" method="POST" class="space-y-3">
                                ...
                                @method('PUT')
                                <div>
                                    <label for="youtube_url" class="block text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-1">Tautan Video YouTube / Live Streaming</label>
                                    <div class="flex gap-2">
                                        <input type="url" name="youtube_url" id="youtube_url"
                                            value="{{ old('youtube_url', $invitation->youtube_url) }}"
                                            class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs"
                                    ...
                                    <p class="text-xs text-neutral-400 dark:text-neutral-500 mt-1.5">Mendukung format <span class="font-mono">youtube.com/watch?v=</span>, <span class="font-mono">youtu.be/</span>, <span class="font-mono">youtube.com/live/</span>, atau ID video langsung.</p>
                                    @error('youtube_url') <span class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</span> @enderror
                                </div>
                            </form>

                            <div class="flex items-center justify-between bg-neutral-50 dark:bg-secondary-700 rounded-xl px-4 py-2.5 border border-neutral-200 dark:border-secondary-700">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full {{ $invitation->show_video ? 'bg-green-500 dark:bg-green-400' : 'bg-neutral-300 dark:bg-neutral-600' }}"></div>
                                    <span class="text-xs font-medium text-neutral-700 dark:text-neutral-300">Video {{ $invitation->show_video ? 'ditampilkan' : 'disembunyikan' }} di halaman undangan</span>
                                </div>
                                <a href="{{ route('dashboard.invitations.edit', $invitation) }}#video-section"
                                    class="text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold">Ubah di Edit &rarr;</a>
                            </div>
                            ...
                            @if($invitation->youtube_video_id)
                                ...
                                <div class="flex items-center justify-between">
                                    <a href="https://youtube.com/watch?v={{ $invitation->youtube_video_id }}" target="_blank"
                                        class="inline-flex items-center gap-1.5 text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                                        </svg>
                                        Buka di YouTube
                                    </a>
                                    <span class="text-xs text-neutral-400 dark:text-neutral-500">ID: {{ $invitation->youtube_video_id }}</span>
                                </div>
                            @else
                                <div class="bg-neutral-50 dark:bg-secondary-700 border border-dashed border-neutral-200 dark:border-secondary-700 rounded-xl p-6 text-center">
                                    <div class="w-10 h-10 mx-auto rounded-xl bg-neutral-100 dark:bg-secondary-700 flex items-center justify-center text-neutral-400 dark:text-neutral-500">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                        </svg>
                                    </div>
                                    <p class="mt-2 text-xs font-medium text-neutral-500 dark:text-neutral-400">Belum ada video ditambahkan</p>
                                    <p class="mt-0.5 text-xs text-neutral-400 dark:text-neutral-500">Masukkan tautan YouTube di atas untuk menampilkan video atau siaran langsung di undangan Anda.</p>
                                </div>
                            @endif
                        </div>
                    @else
                        <div class="text-center py-8">
                            <div class="w-14 h-14 mx-auto rounded-2xl bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border overflow-hidden
                    {{ $galleryLocked ? 'border-amber-100 dark:border-amber-800 bg-amber-50/20 dark:bg-amber-900/10' : 'border-neutral-100 dark:border-secondary-700' }}">
                    <div class="p-5">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-sm font-semibold text-secondary-800 dark:text-neutral-100 flex items-center gap-2">
                                <svg class="w-4 h-4 text-primary dark:text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @if(!$galleryLocked)
                                <span class="text-xs text-neutral-500 dark:text-neutral-400 font-semibold">{{ count($invitation->gallery_photos ?? []) }} / {{ $invitation->maxGalleryPhotos() }} Foto</span>
                            @endif
                        </div>
                        ...
                        @if(!$galleryLocked)
                            <div class="space-y-6">
                                <form id="gallery-upload-form" action="{{ route('dashboard.invitations.gallery.update', $invitation) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                    @csrf
                                    <div id="gallery-dropzone" class="relative border-2 border-dashed border-primary-300 dark:border-primary-700 rounded-2xl p-6 text-center cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/20 transition-all duration-200">
                                        ...
                                        <div id="dropzone-empty" class="space-y-2">
                                            <div class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-primary-100 dark:bg-primary-900/50 text-primary-600 dark:text-primary-400">
                                                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                                </svg>
                                            </div>
                                            <p class="text-sm font-medium text-neutral-700 dark:text-neutral-300">Seret foto ke sini atau <span class="text-primary-600 dark:text-primary-400 underline">klik untuk memilih</span></p>
                                            <p class="text-xs text-neutral-400 dark:text-neutral-500">Format gambar apa pun. Akan dikonversi ke WebP otomatis.</p>
                                        </div>
                                        <div id="dropzone-preview" class="hidden space-y-3">
                                            ...
                                            <div class="flex items-center justify-center gap-2 text-sm text-neutral-500 dark:text-neutral-400">
                                                ...
                                                <button type="button" id="gallery-change-files" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 underline text-xs">Ganti pilihan</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center justify-end gap-3">
                                        <span id="dropzone-error" class="text-xs text-red-500 dark:text-red-400 hidden"></span>
                                ...
                                @if(empty($invitation->gallery_photos))
                                    <p class="text-neutral-500 dark:text-neutral-400 text-center py-4 text-sm">Belum ada foto galeri.</p>
                                @else
                                    <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                                        @foreach($invitation->gallery_photos as $index => $photo)
                                            <div class="relative group aspect-square rounded-xl overflow-hidden border border-neutral-100 dark:border-secondary-700 bg-neutral-50 dark:bg-secondary-700">
                ...
                {{-- Gift --}}
                <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border overflow-hidden
                    {{ $invitation->canUseGift() ? 'border-neutral-100 dark:border-secondary-700' : 'border-amber-100 dark:border-amber-800 bg-amber-50/20 dark:bg-amber-900/10' }}">
                    <div class="p-5">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-sm font-semibold text-secondary-800 dark:text-neutral-100 flex items-center gap-2">
                                <svg class="w-4 h-4 text-primary dark:text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @if($invitation->canUseGift())
                            ...
                            <form id="gift-form" action="{{ route('dashboard.invitations.gift.update', $invitation) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                @csrf
                                <div class="flex items-center justify-between">
                                    <span class="text-xs text-neutral-500 dark:text-neutral-400 font-semibold">Maksimal {{ $maxGift }} akun</span>
                                    <span id="gift-account-count" class="text-xs text-neutral-400 dark:text-neutral-500">0 / {{ $maxGift }}</span>
                                </div>

                                <div id="gift-banks-container" class="space-y-3">
                                    <div class="flex items-center justify-between">
                                        <label class="text-xs font-semibold text-neutral-700 dark:text-neutral-300">Transfer Bank</label>
                                        <button type="button" id="add-bank-btn" class="text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold">+ Tambah Bank</button>
                                    </div>
                                    @foreach($oldBanks as $bankIdx => $bank)
                                        @php $bank = (object) $bank; @endphp
                                        <div class="gift-bank-card bg-neutral-50 dark:bg-secondary-700 p-3 rounded-xl border border-neutral-200 dark:border-secondary-700 space-y-2">
                                            <div class="flex items-center justify-between">
                                                <span class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Bank #{{ $loop->iteration }}</span>
                                                <button type="button" class="remove-bank text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-400 text-xs font-semibold">Hapus</button>
                                            </div>
                                            <div class="grid grid-cols-2 gap-2">
                                                <div>
                                                    <input type="text" name="gift_banks[{{ $bankIdx }}][bank_name]" value="{{ old('gift_banks.' . $bankIdx . '.bank_name', $bank->bank_name ?? '') }}" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nama Bank">
                                                    @error('gift_banks.' . $bankIdx . '.bank_name') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                                </div>
                                                <div>
                                                    <input type="text" name="gift_banks[{{ $bankIdx }}][account_number]" value="{{ old('gift_banks.' . $bankIdx . '.account_number', $bank->account_number ?? '') }}" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="No. Rekening">
                                                    @error('gift_banks.' . $bankIdx . '.account_number') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                                </div>
                                                <div class="col-span-2">
                                                    <input type="text" name="gift_banks[{{ $bankIdx }}][account_holder]" value="{{ old('gift_banks.' . $bankIdx . '.account_holder', $bank->account_holder ?? '') }}" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Atas Nama">
                                                    @error('gift_banks.' . $bankIdx . '.account_holder') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div id="gift-ewallets-container" class="space-y-3">
                                    <div class="flex items-center justify-between">
                                        <label class="text-xs font-semibold text-neutral-700 dark:text-neutral-300">Dompet Digital</label>
                                        <button type="button" id="add-ewallet-btn" class="text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold">+ Tambah E-Wallet</button>
                                    </div>
                                    @foreach($oldEwallets as $ewalletIdx => $ewallet)
                                        @php $ewallet = (object) $ewallet; @endphp
                                        <div class="gift-ewallet-card bg-neutral-50 dark:bg-secondary-700 p-3 rounded-xl border border-neutral-200 dark:border-secondary-700 space-y-2">
                                            <div class="flex items-center justify-between">
                                                <span class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">E-Wallet #{{ $loop->iteration }}</span>
                                                <button type="button" class="remove-ewallet text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-400 text-xs font-semibold">Hapus</button>
                                            </div>
                                            <div class="grid grid-cols-2 gap-2">
                                                <div>
                                                    <input type="text" name="gift_ewallets[{{ $ewalletIdx }}][wallet_name]" value="{{ old('gift_ewallets.' . $ewalletIdx . '.wallet_name', $ewallet->wallet_name ?? '') }}" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nama E-Wallet">
                                                    @error('gift_ewallets.' . $ewalletIdx . '.wallet_name') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                                </div>
                                                <div>
                                                    <input type="text" name="gift_ewallets[{{ $ewalletIdx }}][wallet_number]" value="{{ old('gift_ewallets.' . $ewalletIdx . '.wallet_number', $ewallet->wallet_number ?? '') }}" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nomor E-Wallet">
                                                    @error('gift_ewallets.' . $ewalletIdx . '.wallet_number') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-neutral-700 dark:text-neutral-300">Barcode QRIS</label>
                                    <div class="mt-1 flex items-center gap-4">
                                        @if($invitation->gift_qris_image)
                                            <img src="{{ asset('storage/' . $invitation->gift_qris_image) }}" alt="QRIS" class="w-16 h-16 object-contain border border-neutral-200 dark:border-secondary-700 rounded-xl">
                                        @endif
                                    ...
                                    @error('gift_qris_image') <span class="text-red-500 dark:text-red-400 text-xs">{{ $message }}</span> @enderror
                                </div>
                            ...
                            <template id="gift-bank-template">
                                <div class="gift-bank-card bg-neutral-50 dark:bg-secondary-700 p-3 rounded-xl border border-neutral-200 dark:border-secondary-700 space-y-2">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">Bank Baru</span>
                                        <button type="button" class="remove-bank text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-400 text-xs font-semibold">Hapus</button>
                                    </div>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <input type="text" name="gift_banks[999][bank_name]" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nama Bank">
                                        </div>
                                        <div>
                                            <input type="text" name="gift_banks[999][account_number]" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="No. Rekening">
                                        </div>
                                        <div class="col-span-2">
                                            <input type="text" name="gift_banks[999][account_holder]" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Atas Nama">
                                        </div>
                                    </div>
                                </div>
                            </template>
                            ...
                            <template id="gift-ewallet-template">
                                <div class="gift-ewallet-card bg-neutral-50 dark:bg-secondary-700 p-3 rounded-xl border border-neutral-200 dark:border-secondary-700 space-y-2">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs font-semibold text-neutral-500 dark:text-neutral-400">E-Wallet Baru</span>
                                        <button type="button" class="remove-ewallet text-red-400 dark:text-red-400 hover:text-red-600 dark:hover:text-red-400 text-xs font-semibold">Hapus</button>
                                    </div>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <input type="text" name="gift_ewallets[999][wallet_name]" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nama E-Wallet">
                                        </div>
                                        <div>
                                            <input type="text" name="gift_ewallets[999][wallet_number]" class="block w-full rounded-xl border-neutral-300 dark:border-neutral-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-xs" placeholder="Nomor E-Wallet">
                                        </div>
                                    </div>
                                </div>
                            </template>
                            ...
                            @foreach($invitation->wishes->take(5) as $wish)
                                <div class="border-b border-neutral-100 dark:border-secondary-700 pb-4 last:border-0 last:pb-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        <div class="w-7 h-7 rounded-full bg-primary-100 dark:bg-primary-900/50 flex items-center justify-center text-primary-700 dark:text-primary-300 text-xs font-semibold">
                                            {{ substr($wish->guest_name, 0, 1) }}
                                        </div>
                                        <h4 class="font-semibold text-sm text-secondary-800 dark:text-neutral-100">{{ $wish->guest_name }}</h4>
                                    </div>
                                    <p class="text-sm text-neutral-600 dark:text-neutral-400 ml-9">{{ $wish->message }}</p>
                                    <p class="text-xs text-neutral-400 dark:text-neutral-500 mt-1 ml-9">{{ $wish->created_at->diffForHumans() }}</p>
                                </div>
                            @endforeach
                        </div>
                    @endif
            ...
            <div class="bg-white dark:bg-secondary-800 rounded-2xl shadow-soft border border-neutral-100 dark:border-secondary-700 overflow-hidden">
                <div class="p-5">
                    ...
                    @if($invitation->rsvps->isEmpty())
                        <p class="text-neutral-500 dark:text-neutral-400 text-center py-4 text-sm">Belum ada konfirmasi kehadiran dari tamu.</p>
                    @else
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-neutral-200 dark:divide-secondary-700">
                                <thead class="bg-neutral-50 dark:bg-secondary-700">
                                    <tr>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Nama Tamu</th>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Kehadiran</th>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Jumlah (Pax)</th>
                                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Waktu</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-secondary-800 divide-y divide-neutral-100 dark:divide-secondary-700">
                                    @foreach($invitation->rsvps->take(10) as $rsvp)
                                        <tr class="hover:bg-neutral-50 dark:hover:bg-secondary-700 transition-colors">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-secondary-800 dark:text-neutral-100">{{ $rsvp->guest_name }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold
                                                    {{ $rsvp->attendance === 'attending' ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300' :
                                                      ($rsvp->attendance === 'not_attending' ? 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300' : 'bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300') }}">
                                                    {{ $rsvp->attendanceLabel() }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400">{{ $rsvp->pax }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">{{ $rsvp->created_at->format('d/m/Y H:i') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
