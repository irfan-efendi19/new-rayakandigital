...
<div :style="order === 'male_first' ? { order: 1 } : { order: 2 }"
    ...
    <div
        ...
        <div
            class="w-8 h-8 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 flex items-center justify-center text-blue-500 dark:text-blue-400 flex-shrink-0">
        ...
        <div class="photo-upload-zone flex items-center gap-4 border-2 border-dashed border-neutral-200 dark:border-secondary-700 rounded-2xl p-4 bg-white dark:bg-secondary-800/60 hover:border-primary-400 dark:hover:border-primary-600 hover:bg-primary-50/20 dark:hover:bg-primary-950/10 transition-all cursor-pointer group"
            ...
            <div class="flex-1 min-w-0">
                ...
                <input type="file" name="groom_photo" id="groom_photo_input"
                    ...
                    data-preview="groom-preview" data-title="Foto Mempelai Pria"
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
            <label for="groom_name" class="form-label-crafted">Nama Lengkap <span
                    class="text-red-500">*</span></label>
            <input type="text" name="groom_name" id="groom_name"
                value="{{ old('groom_name') }}" class="form-input-crafted" required
                placeholder="Budi Santoso">
            @error('groom_name') <span
            class="text-red-500 dark:text-red-400 text-xs mt-1 block font-medium">{{ $message }}</span>@enderror
        </div>
        <div>
            <label for="groom_nickname" class="form-label-crafted">Nama
                Panggilan</label>
            <input type="text" name="groom_nickname" id="groom_nickname"
                value="{{ old('groom_nickname') }}" class="form-input-crafted"
                placeholder="Budi">
            @error('groom_nickname') <span
            class="text-red-500 dark:text-red-400 text-xs mt-1 block font-medium">{{ $message }}</span>@enderror
        </div>
        <div>
            <label for="groom_father_name" class="form-label-crafted">Nama Ayah
                <span class="text-red-500">*</span></label>
            <input type="text" name="groom_father_name" id="groom_father_name"
                value="{{ old('groom_father_name') }}" class="form-input-crafted"
                placeholder="Nama Ayah Mempelai Pria" required>
            @error('groom_father_name') <span
            class="text-red-500 dark:text-red-400 text-xs mt-1 block font-medium">{{ $message }}</span>@enderror
        </div>
        <div>
            <label for="groom_mother_name" class="form-label-crafted">Nama Ibu <span
                    class="text-red-500">*</span></label>
            <input type="text" name="groom_mother_name" id="groom_mother_name"
                value="{{ old('groom_mother_name') }}" class="form-input-crafted"
