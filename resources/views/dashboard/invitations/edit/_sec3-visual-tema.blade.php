...
{{-- ======================================== --}}
<div id="sec-3" x-show="activeSection === 'sec-3'" x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0"
    class="scroll-mt-32" data-tour="layar-sapa-config" x-cloak>
    <div class="mb-3">
        <h3 class="font-heading text-lg font-bold text-secondary-800 dark:text-neutral-100">
            Visual & Estetika <span class="text-sm font-normal text-neutral-400 dark:text-neutral-500">(Foto &
                Tema)</span>
        </h3>
    </div>
    ...
    <div data-tour="cover-photo">
        <h4 class="font-heading text-base font-bold text-secondary-800 dark:text-neutral-100 mb-1">
            Foto Sampul</h4>
        ...
        <div
            ...
            <div>
                <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Foto
                    Sampul</label>
                <div class="mt-2 flex flex-col sm:flex-row items-start gap-4">
                    ...
                    <div>
                        <input type="file" name="cover_photo" id="cover_photo_input" class="crop-file-input hidden"
                            ...
                            data-height="640" data-title="Foto Sampul (Cover)">
    ...
    @if($invitation->hasFeature('youtube_video'))
        <div data-tour="youtube-video"
            ...
            <div>
                <label for="youtube_url" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Tautan
                    Video YouTube</label>
                ...
                @error('youtube_url') <span class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</span>
                @enderror
            </div>
                    ...
                    <div id="gallery-dropzone"
                        ...
                        <div id="dropzone-empty" class="space-y-2">
                            ...
                            <p class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                Seret foto ke
                                sini atau <span class="text-primary-600 dark:text-primary-400 underline">klik
                                    untuk
                                    memilih</span>
                            </p>
        ...
        @else
            <div class="text-center py-6">
                <div
                    ...
                    <svg class="w-6 h-6 text-amber-500 dark:text-amber-400" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                    </svg>
    ...
    <div data-tour="music-background" class="mt-6">
        <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Musik
                ...
                <div
                    ...
                    <label
                        ...
                        <input type="radio" name="use_custom_music" value="0"
                            class="h-4 w-4 text-primary focus:ring-primary-500" {{ !$invitation->use_custom_music ? 'checked' : '' }}>
                        <div>
                            <span class="text-sm font-semibold text-secondary-800 dark:text-neutral-200">Gunakan Musik
                                Bawaan Tema</span>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Putar musik default dari tema
                                yang dipilih</p>
                        </div>
                    </label>
                    <label
                        ...
                        <input type="radio" name="use_custom_music" value="1"
                            class="h-4 w-4 text-primary focus:ring-primary-500" {{ $invitation->use_custom_music ? 'checked' : '' }}>
                        <div>
                            <span class="text-sm font-semibold text-secondary-800 dark:text-neutral-200">Gunakan Lagu
                                Sendiri</span>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Unggah file MP3/audio dari
                                perangkat Anda</p>
                        </div>
                    </label>
                </div>
    ...
    {{-- Theme Selection --}}
    @php
        $currentTheme = old('theme', $invitation->theme);
        ...
        $initialCategory = $themes->firstWhere('slug', $currentTheme)?->catalog_category_slug
            ?? $defaultCategory;
    @endphp
    <div x-data="themePicker" data-selected-theme="{{ $currentTheme }}" data-initial-category="{{ $initialCategory }}"
        class="mt-6 space-y-3" data-tour="select-theme">
        ...
        <div class="flex flex-col">
            ...
            <span class="text-[11px] text-neutral-400 mt-0.5">
                Pilih kategori untuk mempersempit koleksi, lalu klik kartu tema yang
                ingin diaktifkan. Pratinjau lain dimuat saat diperlukan.
            </span>
        </div>

        <div class="flex gap-2 overflow-x-auto px-1 pb-1 scrollbar-thin" role="group" aria-label="Filter kategori tema">
            ...
            @foreach($themeCategories as $category)
                <button type="button" data-category="{{ $category['slug'] }}" @click="setCategory($el.dataset.category)"
        ...
        <div x-ref="themeList"
            ...
            @foreach($themes as $tema)
                ...
                <button type="button" data-theme="{{ $themeKey }}" data-category="{{ $tema->catalog_category_slug }}"
                    @click="selectTheme($el.dataset.theme)" x-show="showsCategory($el.dataset.category)" x-cloak
                    ...
                    <div
                        ...
                        @if($tema->thumbnail_url)
                            <img src="{{ $tema->thumbnail_url }}" loading="lazy" decoding="async"
        ...
        @error('theme') <span class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</span>
        @enderror
    </div>
</div>