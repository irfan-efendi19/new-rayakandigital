            ...
            <div class="bg-white dark:bg-secondary-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8">
                    ...
                    <div class="text-center mb-8">
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-neutral-100">Tagihan Pembayaran</h3>
                    ...
                    <div class="bg-gray-50 dark:bg-secondary-700/50 rounded-xl p-6 mb-6 border border-gray-200 dark:border-secondary-600">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-neutral-400 uppercase tracking-wide">ID Invoice</p>
                                <p class="text-lg font-bold text-gray-900 dark:text-neutral-100 font-mono">{{ $order->invoice_id }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-neutral-400 uppercase tracking-wide">Paket</p>
                                <p class="text-lg font-bold text-gray-900 dark:text-neutral-100">{{ ucfirst($order->package_type) }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-neutral-400 uppercase tracking-wide">Status</p>
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                                    {{ $order->payment_status === 'pending' ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300' : '' }}
                                    {{ $order->payment_status === 'verifying' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300' : '' }}
                                    {{ $order->payment_status === 'success' ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : '' }}
                                ">
                    ...
                    @elseif($order->payment_status === 'success')
                        <div class="text-center bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6">
                            ...
                            <p class="text-sm text-green-600 dark:text-green-400 mt-1">Paket {{ ucfirst($order->package_type) }} Anda sudah aktif.</p>
                        </div>
                    @endif

                    <div class="mt-6 text-center">
                        <a href="{{ route('dashboard') }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 text-sm font-medium">
