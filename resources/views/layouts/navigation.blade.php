        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                        ...
                        <span class="font-sans text-xl font-bold text-secondary-800 dark:text-neutral-100 group-hover:text-primary transition-colors duration-300">
                            Digital
                        </span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-1 sm:-my-px sm:ms-10 sm:flex sm:items-center">
                    ...
                    @if(Auth::user()->isAdmin())
                        <x-nav-link href="{{ route('admin.impersonate.index') }}" :active="request()->routeIs('admin.impersonate.*')">
                            Intip Pengguna
                        </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center gap-3 px-4 py-2 rounded-xl border border-neutral-200 dark:border-secondary-600 bg-white dark:bg-secondary-800 hover:bg-neutral-50 dark:hover:bg-secondary-700 hover:border-primary-200 dark:hover:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-secondary-900 transition-all duration-200">
                            <div class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center text-primary-700 dark:text-primary-300 font-semibold text-sm overflow-hidden">
                                @if(Auth::user()->avatar)
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-3 border-b border-neutral-100 dark:border-secondary-700 lg:hidden">
                            <p class="text-sm font-medium text-secondary-800 dark:text-neutral-200">{{ Auth::user()->name }}</p>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{{ Auth::user()->email }}</p>
                        </div>
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden dark:bg-secondary-900">
        <div class="pt-2 pb-3 space-y-1 border-t border-neutral-200 dark:border-secondary-700">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 text-neutral-400 dark:text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                    </svg>
                    Dashboard
                </div>
        ...
        <div class="pt-4 pb-3 border-t border-neutral-200 dark:border-secondary-700">
            <div class="px-4">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center text-primary-700 dark:text-primary-300 font-semibold overflow-hidden">
                        @if(Auth::user()->avatar)
                            <img src="{{ Auth::user()->avatar }}" alt="" class="w-full h-full object-cover">
                        @else
                            {{ substr(Auth::user()->name, 0, 1) }}
                    ...
                    <div>
                        <p class="font-medium text-sm text-secondary-800 dark:text-neutral-200">{{ Auth::user()->name }}</p>
