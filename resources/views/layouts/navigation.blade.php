        ...
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-1 sm:-my-px sm:ms-8 sm:flex sm:items-center h-16">
                    @php
                        $navLinks = [
                            ...
                            ['url' => route('dashboard.invitations.create'), 'route' => 'dashboard.invitations.create', 'label' => 'Buat Undangan', 'icon' => 'M12 4v16m8-8H4'],
                            ['url' => route('dashboard.checkout'), 'route' => 'dashboard.checkout', 'label' => 'Paket & Harga', 'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'],
                        ];
                    @endphp
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                ...
                <button type="button"
                    ...
                    <svg x-show="isDark" class="w-5 h-5 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="display:none;">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                    {{-- Moon Icon (shown in light mode) --}}
                    <svg x-show="!isDark" class="w-5 h-5 text-neutral-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @auth
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" @click.away="open = false"
                            ...
                            {{-- Avatar Initial --}}
                            <div class="w-7 h-7 rounded-lg bg-gradient-to-tr from-primary-600 to-primary-400 text-white font-bold flex items-center justify-center text-xs shadow-sm">
                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                            </div>
                            <span class="max-w-[120px] truncate font-semibold text-xs">{{ Auth::user()->name }}</span>
                            @if(Auth::user()->is_admin)
                                <span class="px-1.5 py-0.5 rounded text-[10px] font-extrabold bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-300 uppercase tracking-wider">Admin</span>
                            @endif
                            <svg class="w-4 h-4 text-neutral-400 transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div x-show="open"
                            ...
                            <div class="px-4 py-2.5">
                                ...
                                <p class="text-sm font-bold text-secondary-800 dark:text-neutral-100 truncate">{{ Auth::user()->email }}</p>
                            </div>
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm font-semibold text-neutral-600 dark:text-neutral-300 hover:text-primary">Masuk</a>
                    <a href="{{ route('register') }}" class="px-4 py-2 rounded-xl bg-primary hover:bg-primary-600 text-white text-sm font-semibold transition-all">Daftar</a>
                @endauth
            ...
            <div class="-me-2 flex items-center sm:hidden">
                ...
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-xl text-neutral-400 hover:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-secondary-800 focus:outline-none transition duration-150 ease-in-out">
        ...
        @auth
            <div class="pt-4 pb-3 border-t border-neutral-200 dark:border-secondary-700 px-4">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="font-bold text-base text-secondary-800 dark:text-neutral-100">{{ Auth::user()->name }}</div>
                        <div class="font-medium text-xs text-neutral-500 dark:text-neutral-400">{{ Auth::user()->email }}</div>
                    </div>
                </div>
                ...
                <div class="mt-3 space-y-1">
                    ...
                    @if(Auth::user()->is_admin)
                        ...
                        <x-responsive-nav-link :href="route('admin.impersonate.index')" class="text-amber-600 dark:text-amber-400 font-bold">
                            Intip Pengguna
                        </x-responsive-nav-link>
                    @endif
