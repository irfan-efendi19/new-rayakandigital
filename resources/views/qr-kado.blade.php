    ...
    <div class="qr-content">
        @if ($giftBanks->isNotEmpty() || $giftEwallets->isNotEmpty() || $qrisUrl)
            @if ($giftBanks->isNotEmpty() || $giftEwallets->isNotEmpty())
                <section class="qr-section qr-panel" aria-labelledby="gift-accounts-title">
                    <div class="qr-section__heading">
                        <div>
                            <p class="qr-label">Transfer</p>
                            <h2 id="gift-accounts-title" class="qr-section-title">Pilih tujuan</h2>
                        </div>
                        <span class="qr-count">{{ $giftBanks->count() + $giftEwallets->count() }} metode</span>
                    </div>
            ...
            @if ($qrisUrl)
                <section class="qr-section qr-panel qr-qris" aria-labelledby="qris-title">
                    <div>
                    ...
                    <button type="button" class="qr-qris__preview" data-qris-open aria-label="Perbesar kode QRIS">
                        <img src="{{ $qrisUrl }}" alt="Kode QRIS untuk {{ $invitation->couple_name }}">
                        <span>Ketuk untuk memperbesar</span>
                    </button>
                </section>

                <dialog class="qr-dialog" data-qris-dialog aria-labelledby="qris-dialog-title">
                    <div class="qr-dialog__bar">
                        <p id="qris-dialog-title" class="text-sm font-bold">QRIS · {{ $invitation->couple_name }}</p>
                        <button type="button" class="qr-button qr-button--secondary qr-button--small" data-qris-close>Tutup</button>
                    </div>
        ...
        @else
            <div class="qr-empty">
                <span class="qr-state-icon"><x-qr-icon name="gift" /></span>
